...
public class ClearInventoryCommand extends BaseCommand {
    public ClearInventoryCommand(EventTools plugin) {
        super(plugin);
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length != 1) {
            plugin.sendMessage(sender, "&cUsage: /clearinventory <player|all|alive|eliminated|@team>");
            return true;
        }
        ...
        targets.forEach(player -> {
            ...
            plugin.sendMessage(player, "&cYour inventory was cleared!");
        });

        String targetName = formatTargetName(args[0], targets.size());
        plugin.sendMessage(sender, String.format("&eCleared inventory of %s!", targetName));
        return true;
    }
    ...
    private void handleNoTargets(CommandSender sender, String target) {
        if (target.equalsIgnoreCase(sender.getName())) {
            plugin.sendMessage(sender, "&cYou can't clear your own inventory!");
        } else if (getTargetPlayers(sender, target).stream().anyMatch(p -> p.hasPermission("eventtools.bypass"))) {
            plugin.sendMessage(sender, "&cYou can't clear the inventory of that player!");
        } else {
