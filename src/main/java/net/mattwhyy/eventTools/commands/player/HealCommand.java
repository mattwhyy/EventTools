...
public class HealCommand extends BaseCommand {
    public HealCommand(EventTools plugin) {
        super(plugin);
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length != 1) {
            plugin.sendMessage(sender, "&cUsage: /heal <player|all|alive|eliminated|@team>");
            return true;
        }
        ...
        targets.forEach(player -> {
            plugin.healPlayer(player);
            plugin.sendMessage(player, "&aYou have been healed!");
        });

        String targetName = formatTargetName(args[0], targets.size());
        plugin.sendMessage(sender, String.format("&aHealed %s!", targetName));
        return true;
    }
    ...
    private void handleNoTargets(CommandSender sender, String target) {
        if (target.equalsIgnoreCase(sender.getName())) {
            plugin.sendMessage(sender, "&cYou can't heal yourself!");
        } else if (getTargetPlayers(sender, target).stream().anyMatch(p -> p.hasPermission("eventtools.bypass"))) {
            plugin.sendMessage(sender, "&cYou can't heal that player!");
        } else {
